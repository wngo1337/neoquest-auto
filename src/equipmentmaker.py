...
class EquipmentMaker:
    ...
    ITEM_MAKER_ADDRESS_TEMPLATE = "http://www.neopets.com/games/neoquest/neoquest.phtml?action=talk&target={}&say=&give=1"
    EQUIP_ADDRESS_TEMPLATE = "http://www.neopets.com/games/neoquest/neoquest.phtml?action=items&equipitemid={}&do=equip"
    ...
    def set_equipment_recipe(self, equipment_name):
        ...
        self.equipment_id, self.maker_id, self.recipe = (
            gameinfo.equipment_names_to_recipes[equipment_name]
        )
        ...
        if self.are_ingredients_present():
            ...
            for ingredient in self.recipe:
                ...
                seleniumobjectsandmethods.click_link_by_xpath(
                    path_template.format(self.get_ingredient_id(ingredient))
                )
            seleniumobjectsandmethods.click_link_by_xpath(
                "//INPUT[contains(@VALUE, 'Give items to')]"
            )
    ...
    def equip_equipment(self):
        seleniumobjectsandmethods.go_to_url(constants.ITEM_PAGE_URL)
        seleniumobjectsandmethods.go_to_url(
            self.EQUIP_ADDRESS_TEMPLATE.format(self.equipment_id)
        )
