...
#         # These are the numbers that correspond with Life Weapons, Field Medic, and Lifesteal
        ...
        levels_to_skill_investments.update(dict.fromkeys([-7, -4, -1, 4, 7, 10, 13, 16, 19, 22], 5001))
        levels_to_skill_investments.update(dict.fromkeys([-6, -3, 2, 5, 8, 11, 14, 17, 20, 23], 5002))
        levels_to_skill_investments.update(dict.fromkeys([-5, -2, 3, 6, 9, 12, 15, 18, 21, 24], 5003))
#
#     # TODO: There has to be a better way to build the dictionary...
    levels_to_skill_investments.update(dict.fromkeys([25, 29, 33, 37, 41, 45, 49], "3001"))
    levels_to_skill_investments.update(dict.fromkeys([26, 30, 34, 38, 42, 46, 50], "3002"))
    ...
    def initial_skill_setup(self):
        ...
        for i in range(-7, 0):
            seleniumobjectsandmethods.go_to_url(
                self.SKILL_SELECT_ADDRESS_TEMPLATE.format(self.levels_to_skill_investments[i]))
        # Really don't want to bother putting these in a file since they are only used here...
        seleniumobjectsandmethods.go_to_url("http://www.neopets.com/games/neoquest/neoquest.phtml?cc_accept=1")
        seleniumobjectsandmethods.go_to_url("http://www.neopets.com/games/neoquest/neoquest.phtml?weapon_choice=5")
        seleniumobjectsandmethods.go_to_url("http://www.neopets.com/games/neoquest/neoquest.phtml?cc_accept=1")
        seleniumobjectsandmethods.go_to_url("http://www.neopets.com/games/neoquest/neoquest.phtml?cc_accept=1")
        seleniumobjectsandmethods.go_to_url("http://www.neopets.com/games/neoquest/neoquest.phtml")

    def has_points(self):
        try:
            seleniumobjectsandmethods.single_driver.find_element(By.XPATH, self.SPEND_POINTS_XPATH)
    ...
    def spendPoints(self, level):
        ...
        seleniumobjectsandmethods.go_to_url(self.SKILL_SELECT_ADDRESS_TEMPLATE.format(skill_id))
        seleniumobjectsandmethods.go_to_url(self.SKILL_CONFIRM_ADDRESS_TEMPLATE.format(skill_id))
